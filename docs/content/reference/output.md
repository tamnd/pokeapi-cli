...
describes what users get. Pick a format with `-o`, or let pokeapi choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
pokeapi <command> -o table   # aligned columns for reading
pokeapi <command> -o jsonl   # one JSON object per line, for piping
pokeapi <command> -o json    # a single JSON array
pokeapi <command> -o csv     # spreadsheet friendly
pokeapi <command> -o tsv     # tab-separated
pokeapi <command> -o url     # just the URL column
pokeapi <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
pokeapi <command> --fields id,title,url
```
...
```bash
pokeapi <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
pokeapi <command>            # a table, because this is a terminal
pokeapi <command> | wc -l    # JSONL, because this is a pipe
```
