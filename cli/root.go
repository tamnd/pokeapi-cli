// Package cli assembles the pokeapi command tree from the pokeapi
...
// NewApp assembles the kit application from the pokeapi domain. The
...
//
// To add a command, declare it in pokeapi/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := pokeapi.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(pokeapi.Domain{}).Register(app)
